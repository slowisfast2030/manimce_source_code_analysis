...
class test_2(Scene):
    def construct(self):
        ...
        """
        vm = VMobject()
        outer_circle = Circle(radius=2.1).rotate(PI/2).get_points()[:16]
        inner_circle = Circle(radius=2).rotate(PI/2).get_points()[:16][::-1]
...
class test(Scene):
    def construct(self):
        ...
        """
        vm = VMobject()
        outer_circle = Circle(radius=3).points[:3]
        inner_circle = Circle(radius=2).points[:3]
        ...
        for index, point in enumerate(vm.points):
            dot = Dot(point)
            if index < 21:
        ...
        for index, point in enumerate(rec.points):
            dot = Dot(point)
            if index < 21:
                label = Text(str(index), font_size=24).next_to(dot, DOWN, buff=0.1)
            else:
